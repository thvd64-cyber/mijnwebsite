...
/* Timeline module met HoofdID integratie
   ...
   - Automatische eerste render + fallback demo
*/

// ======================= DATA LADEN =======================
let peopleData = window.StamboomStorage?.get() || [];

// Fallback demo-data als storage leeg
...
function parseDateSafe(d){ 
    ...
    const dt = new Date(d);
    return isNaN(dt.getTime()) ? new Date(") : dt;  // fallback ook vandaag bij ongeldige datum
}

function yearToX(year, minYear, maxYear){ 
    const range = maxYear - minYear || 1; // voorkom deling door 0
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(typeof initLiveSearch === "function"){
        initLiveSearch(input, peopleData, selectedPerson => {
            // 🔹 Direct persoon-object gebruiken, geen find() nodig
            if(selectedPerson) drawTimeline(selectedPerson);
        });
    }
